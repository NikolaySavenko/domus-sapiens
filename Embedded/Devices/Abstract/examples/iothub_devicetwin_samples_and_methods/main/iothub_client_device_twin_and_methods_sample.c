...
static GPIOPIN* parseFromJsonPIN(const char* json)
{
    GPIOPIN* GPIO_ = malloc(sizeof(GPIOPIN));
    JSON_Value* root_value = NULL;
    JSON_Object* root_object = NULL;
    ...
    {
        (void)memset(GPIO_, 0, sizeof(GPIOPIN));
        ...
        pin = json_object_get_value(root_object, "pin");
        value = json_object_get_value(root_object, "value");
        
        if (pin != NULL)
        {
            GPIO_->pin = json_value_get_number(pin);
            if(GPIO_->pin==23)
                printf("LOL");
        }
        ...
        if (value != NULL)
        {
            GPIO_->value= json_value_get_number(value);
        }
...
static int deviceMethodCallback(const char* method_name, const unsigned char* payload, size_t size, unsigned char** response, size_t* response_size, void* userContextCallback)
{
    ...
    int result;
    if (strcmp("GPIO_SWITCH", method_name) == 0)
    {
        const char deviceMethodResponse[] = "{ \"Response\": \"1HGCM82633A004352\" }";
        *response_size = sizeof(deviceMethodResponse)-1;
        *response = malloc(*response_size);
        GPIO_ = parseFromJsonPIN((const char*) payload);

        gpio_pad_select_gpio(GPIO_->pin);
        gpio_set_direction(GPIO_->pin, GPIO_MODE_OUTPUT);
        gpio_set_level(GPIO_->pin, GPIO_->value);
        //gpio_pad_select_gpio(23);
        //gpio_set_direction(23, GPIO_MODE_OUTPUT);
        //gpio_set_level(23, GPIO_->value);
